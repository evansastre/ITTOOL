...
;~ MTR("122.198.64.131","1119","Agent1")  ;魔兽
;~ MTR("114.113.217.193","1119","Agent2")  ;魔兽
;~ MTR("122.198.64.138","1119","魔兽世界")  ;魔兽
;~ MTR("114.113.218.136","3724","炉石传说")  ;炉石
;~ MTR("223.252.248.238","1119","暗黑破坏神Ⅲ ")  ;暗黑
;~ MTR("223.252.252.76","3724","守望先锋")  ;守望
;~ MTR("122.198.64.132","1119","星际争霸Ⅱ")  ;星际
;~ MTR("122.198.64.133","1119","风暴英雄")  ;风暴

Func MTR($ChosenAddress,$myports,$now_choose_tool)
	
;~ 	MsgBox(0,"",$ChosenAddress&@LF&$myports&@LF&$now_choose_tool)
...
;~ 	MsgBox(0,"","1")
	...
	TcpingTest($ChosenAddress,$myports,$now_choose_tool)
EndFunc


Func TcpingTest($ChosenAddress,$myports,$now_choose_tool)
	_RunDos("set path=%temp%;%path% && tcping -n 100 " & $ChosenAddress  & "  " & $myports & ">> %temp%\tcpingRresult.txt")
	FileWriteLine(@TempDir & '\tcpingRresult.txt',@LF & "分析：返回100个结果。一般我们认为返回时间平均在30ms以下、无过高峰值判定为良好。" & @LF) ;
	FileWriteLine(@TempDir & '\tcpingRresult.txt',"===================================================================================================") ;
	$hFileOpen=FileOpen(@TempDir & '\tcpingRresult.txt')
	Local $sFileRead = FileRead($hFileOpen)
	_FileWriteToLine(@TempDir & '\winMTRresult.txt',1,$sFileRead) ;写入读取的内容
	FileClose($hFileOpen)
	_FileWriteToLine(@TempDir & '\winMTRresult.txt',1,"===================================================================================================" )
	_FileWriteToLine(@TempDir & '\winMTRresult.txt',2,$now_choose_tool & "  tcping测试：" )
...
Func Logs()
	...
	_FileWriteToLine(@TempDir & '\winMTRresult.txt',1,$endtime) ;写入读取
	FileWriteLine(@TempDir & 'winMTRresult.txt'," ") ;写入空行
