...
#AccAu3Wrapper_OutFile_x64=DriversInstall_win7.exe
#AccAu3Wrapper_Res_Language=2052
#AccAu3Wrapper_Res_requestedExecutionLevel=None
#EndRegion ;**** 由 AccAu3Wrapper_GUI 创建指令 ****
#include <winAPI.au3>
#include <math.au3>
;~ $path="C:\Users\TestUser1\Desktop\WanDrv6_Win7.x64\"  ; for test
...
;~ 	$path='\\shcw01\WanDrv6_Win7.x64\'  ; for test
	
	If Not ProcessExists("WanDrv6(Win7.x64).exe") Then
		ShellExecute($path&"WanDrv6(Win7.x64).exe")
	Else
		;
		WinSetState ( "万能驱动助理 (Win7.x64 专用) - IT天空出品","",@SW_ENABLE )
		WinActivate("万能驱动助理 (Win7.x64 专用) - IT天空出品","")
	EndIf

	;mainWindow
	WinWaitActive("万能驱动助理 (Win7.x64 专用) - IT天空出品","")
	Local $ProcessId = WinGetHandle("万能驱动助理 (Win7.x64 专用) - IT天空出品", "开始")
...
Func judgeLocation()
	...
	If $sh==_Min( $sh, $hz ) Then  ;  The smaller the value, the faster, that is, the closer
		$location = 'SH'
		$file_server='\\shcw01\WanDrv6_Win7.x64\'
	ElseIf $hz==_Min ( $sh, $hz ) Then 
		$location = 'HZ'
		$file_server='\\hzcw01\WanDrv6_Win7.x64\'
	EndIf
...
;~ Local $hWnd=WinWait("万能驱动助理 (Win7.x64 专用) - IT天空出品","")
;~ Local $hWnd=WinWait("万能驱动助理 (Win7.x64 专用) - IT天空出品","")
;~ MsgBox(0,"",$hWnd)
;~ MsgBox(0,"","111")
