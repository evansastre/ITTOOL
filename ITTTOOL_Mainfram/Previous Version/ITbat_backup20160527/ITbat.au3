...
;~ 	GUICtrlSetState($backgroud, $GUI_SHOW)
	...
	Global $play_next_button = GUICtrlCreateIcon($play_next_ico, -1, 75, 13, 38, 26)
	GUICtrlSetResizing(-1, 2 + 768)
	GUICtrlSetTip(-1, "play next")
	GUICtrlSetOnEvent($play_next_button, "play_next_button")
	GUICtrlSetState($play_next_button, $GUI_SHOW)
	
	Global $play_label = GUICtrlCreateLabel(". ",  125, 13, 120, 30)
;~ 	GUICtrlSetResizing(-1, 2 + 768)
;~ 	GUICtrlSetTip(-1, "play next")
;~ 	GUICtrlSetOnEvent($play_next_button, "play_next_button")
	GUICtrlSetState($play_label, $GUI_SHOW)
...
;~ 	Global $idTab = GUICtrlCreateTab(10, 50, 780, 500, BitOR($TCS_MULTILINE, $TCS_RIGHTJUSTIFY), $WS_EX_WINDOWEDGE);$WS_EX_DLGMODALFRAME    $TCS_SCROLLOPPOSITE
	Global $idTab = GUICtrlCreateTab(10, 50, 780, 500, BitOR($TCS_MULTILINE, $TCS_RIGHTJUSTIFY));$WS_EX_DLGMODALFRAME    $TCS_SCROLLOPPOSITE
...
;~ 	$end_time=$start_time
	...
	While 1
		...
		If _SoundPos($aSound, 2) >= $soundlenth Then 
			play_next_button()
		EndIf
...
Func click_Tab()
	...
	MsgBox(0,"",$Tab_curSel)
	_GUICtrlTab_HighlightItem($tabID, 1)
EndFunc
...
;~ 	If FileExists(@WindowsDir & "\Cursors\aero_link.cur") Then
;~ 		_SetCursor(@WindowsDir & "\Cursors\aero_link.cur", $OCR_HAND)
;~ 	Else
;~ 		_SetCursor($select_oringin_ico, $OCR_HAND)
;~ 	EndIf
