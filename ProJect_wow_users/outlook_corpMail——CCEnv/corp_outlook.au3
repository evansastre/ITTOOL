...
#AccAu3Wrapper_OutFile=Y:\test\test.exe
...
Func OACSETUP(Const $SERVERTYPE)
	...
	If $SERVERTYPE = "pop3" Then
		...
		FileInstall(".\pop3_2013.PRF", @TempDir & "\ap_2013.PRF", 1)
		FileInstall(".\pop3_2016.PRF", @TempDir & "\ap_2016.PRF", 1)
;~ 	Else
...
;~ 	If $G_OUTLOOKVER == "2010" Or $G_OUTLOOKVER == "2007" Then
;~ 		WinWaitActive("Microsoft Outlook","")   ;
;~ 	ElseIf $G_OUTLOOKVER == "2016" Then
;~ 		WinWaitActive("[Class:rctrl_renwnd32]","",10)   ;
;~ 	EndIf

	WinWaitActive("[Class:rctrl_renwnd32]","",10)
	Send("{F9}")
	
;~ 	MsgBox(0,"",$G_OUTLOOKVER)
	
	If $EMAILDOMAIN == "corp.corpname.com" Then
	...
	;~ 	MsgBox(0,"",$waitRes)
		...
		If $waitRes <> 0    Then
			WinActive($Title,"")
			ControlSetText($Title,"","RichEdit20WPT2",getCorpPSW($EMAIL)) ;填写密码
...
Func SETPROFILEPROPERTY(Const ByRef $USEREMAIL, Const ByRef $USERACCOUNTNAME)
	...
	$service="Service2"
	;定义默认的存储路径服务 
	IniWrite($PROFILEPATH, "General", "DefaultStore", $service) 
	;service list
	IniWrite($PROFILEPATH, "Service List", $service, "Unicode Personal Folders") 
	
	
	IniWrite($PROFILEPATH, $service, "Name", $USEREMAIL)   ;边栏的邮箱名称，mail.CorpDomain名称强制为"内邮"，corp强制为“企业邮箱”
	IniWrite($PROFILEPATH, $service, "PathAndFilenameToPersonalFolders", "E:\mail\" & $USEREMAIL & ".pst") ; 数据文件名称, wow和corp邮箱用专有名字
	If $EMAILDOMAIN =="mail.CorpDomain" Then
		IniWrite($PROFILEPATH, $service, "Name", "内邮")
		IniWrite($PROFILEPATH, $service, "PathAndFilenameToPersonalFolders", "E:\mail\" & @UserName & "wow.pst") 
	ElseIf $EMAILDOMAIN =="corp.corpname.com" Then
		IniWrite($PROFILEPATH, $service, "Name", "企业邮箱")
		IniWrite($PROFILEPATH, $service, "PathAndFilenameToPersonalFolders", "E:\mail\" & @UserName & "corp.pst") 
	EndIf
	
	
	Local $account="Account1"
	IniWrite($PROFILEPATH, "Internet Account List", $account, "I_Mail")
	IniWrite($PROFILEPATH, $account, "AccountName", $USEREMAIL) ;帐户信息处显示的名称，一般不修改此项，用邮件地址作为名称即可
	IniWrite($PROFILEPATH, $account, "DisplayName", $DISPLAYNAME) ;用户信息————姓名，中文
	IniWrite($PROFILEPATH, $account, "EmailAddress", $USEREMAIL)      ;用户信息————邮件地址
	IniWrite($PROFILEPATH, $account, "ReplyEMailAddress", $USEREMAIL) ;回复地址
	IniWrite($PROFILEPATH, $account, "POP3Server", $EMAILDOMAIN) ;服务器信息————接收邮件服务器
	IniWrite($PROFILEPATH, $account, "SMTPServer", $EMAILDOMAIN)      ;服务器信息————发送邮件服务器
	If ($EMAILDOMAIN <>"mail.CorpDomain" And $EMAILDOMAIN <> "corp.corpname.com" ) Then   ; 非公司内部的邮箱一般收发服务器地址名规则 
		IniWrite($PROFILEPATH, $account, "POP3Server", "pop." & $EMAILDOMAIN)
		IniWrite($PROFILEPATH, $account, "SMTPServer", "smtp." & $EMAILDOMAIN)
	EndIf
	
	If IniRead($PROFILEPATH, $account, "POP3UserName", "") <> "" Then
		If $EMAILDOMAIN =="mail.CorpDomain" Then 
			IniWrite($PROFILEPATH, $account, "POP3UserName", @UserName)      ;登录信息————用户名  pop
		Else
			IniWrite($PROFILEPATH, $account, "POP3UserName", $USEREMAIL)      ;登录信息————用户名  pop
		EndIf
		
	EndIf
	If IniRead($PROFILEPATH, $account, "IMAPUserName", "") <> "" Then    ;登录信息————用户名   imap
		IniWrite($PROFILEPATH, $account, "IMAPUserName", $USEREMAIL)
	EndIf
	...
	If $EMAILDOMAIN <>"mail.CorpDomain" Then             ;非 mail.CorpDomain 都需要勾选发送验证
		...
		IniWrite($PROFILEPATH, "Account1", "SMTPPort", "465")
		IniWrite($PROFILEPATH, "Account1", "SMTPSecureConnection", "1")
...
Func RUN_OUTLOOK()
	...
	Switch $OUTLOOKVER
		...
		Case "2016"
			RegWrite("HKEY_CURRENT_USER\Software\Microsoft\Office\16.0\Outlook\LDAP", "DisableVLVBrowsing", "REG_DWORD", "1")
	EndSwitch
